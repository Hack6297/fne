...
const BlocklyWorkspace = forwardRef(function BlocklyWorkspace(props: Props, ref: any) {
      ...
      (function(b) {
        ...
        Blockly.Blocks[b.id] = {
          init: function(this: any) {
            var json: any = { type: b.id, colour: b.color, tooltip: b.msg };
            if (b.cblock) {
              if (b.hasElse) {
                json.message0 = b.msg; json.args0 = b.args || [];
                json.message1 = '%1'; json.args1 = [{ type: 'input_statement', name: 'DO' }];
                json.message2 = 'else';
                json.message3 = '%1'; json.args3 = [{ type: 'input_statement', name: 'ELSE' }];
                json.previousStatement = null; json.nextStatement = null;
              } else {
                json.message0 = b.msg; json.args0 = b.args || [];
                json.message1 = '%1'; json.args1 = [{ type: 'input_statement', name: 'DO' }];
                json.previousStatement = null; json.nextStatement = null;
            ...
            if (b.hat) { delete json.previousStatement; json.nextStatement = null; }
            this.jsonInit(json);
          }
        };
      })(blocks[i]);
      ...
      for (var i = 0; i < inputList.length; i++) {
        ...
        if (inp.fieldRow) {
          for (var j = 0; j < inp.fieldRow.length; j++) {
            ...
            if (f.name && f.getValue) inputs.push(String(f.getValue()));
          }
        }
      }
      var pos = block.getRelativeToSurfaceXY ? block.getRelativeToSurfaceXY() : { x: 0, y: 0 };
      var pb: PlacedBlock = { uid: block.id, blockId: block.type, inputs: inputs, x: pos.x, y: pos.y, spriteId: currentSpriteIdRef.current };
      if (prevUid) pb.prevUid = prevUid;
      if (parentUid) pb.parentUid = parentUid;
      var nextBlock = block.getNextBlock ? block.getNextBlock() : null;
      if (nextBlock && !processed.has(nextBlock.id)) pb.nextUid = nextBlock.id;
      var doInput = block.getInput ? block.getInput('DO') : null;
      if (doInput && doInput.connection && doInput.connection.targetBlock()) {
        pb.innerUids = [];
        var cur: any = doInput.connection.targetBlock();
        while (cur && !processed.has(cur.id)) { pb.innerUids.push(cur.id); cur = cur.getNextBlock ? cur.getNextBlock() : null; }
      }
      var elseInput = block.getInput ? block.getInput('ELSE') : null;
      if (elseInput && elseInput.connection && elseInput.connection.targetBlock()) {
        pb.elseUids = [];
        var cur2: any = elseInput.connection.targetBlock();
        while (cur2 && !processed.has(cur2.id)) { pb.elseUids.push(cur2.id); cur2 = cur2.getNextBlock ? cur2.getNextBlock() : null; }
      }
      result.push(pb);
      if (doInput && doInput.connection && doInput.connection.targetBlock()) {
        var ib: any = doInput.connection.targetBlock();
        while (ib && !processed.has(ib.id)) { processBlock(ib, undefined, block.id); ib = ib.getNextBlock ? ib.getNextBlock() : null; }
      }
      if (elseInput && elseInput.connection && elseInput.connection.targetBlock()) {
        var eb: any = elseInput.connection.targetBlock();
        while (eb && !processed.has(eb.id)) { processBlock(eb, undefined, block.id); eb = eb.getNextBlock ? eb.getNextBlock() : null; }
      }
      if (nextBlock && !processed.has(nextBlock.id)) processBlock(nextBlock, block.id);
    }
    for (var t = 0; t < allBlocks.length; t++) processBlock(allBlocks[t]);
    return result;
  }
  ...
  function loadWorkspace(ws: any, spriteId: string) {
    try {
      ...
      if (saved && saved.type === 'json') Blockly.serialization.workspaces.load(saved.data, ws);
    } catch (e) {}
  }
  ...
  useImperativeHandle(ref, function() {
    return {
      getAllWorkspaceData: function() {
        if (workspaceRef.current) saveWorkspace(workspaceRef.current, currentSpriteIdRef.current);
  ...
  useEffect(function() {
    ...
    try {
      workspaceRef.current = Blockly.inject(containerRef.current, {
        ...
        trashcan: true, media: '', sounds: false, renderer: 'geras',
      });
    ...
    return function() {
      if (workspaceRef.current) { try { workspaceRef.current.dispose(); } catch (e) {} workspaceRef.current = null; }
