...
var BlocklyWorkspaceComponent = forwardRef<BlocklyWorkspaceHandle, Props>(function BWC(props, ref) {
  ...
  var [st, setSt] = useState<'load'|'ok'|'err'>('load');
    ...
    try {
      ...
      for (var k = 0; k < ids.length; k++) {
        (function(bid: string, d: any) {
          try { delete Blockly.Blocks[bid]; } catch(_) {}
          Blockly.Blocks[bid] = {
      ...
      // NO external media - prevents hanging on localhost
  ...
  // SPRITE SWITCHING - the critical fix
  useEffect(function() {
    var w = wsRef.current;
    if (!w || !didInit.current) return;
    if (curSprite.current === props.selectedSpriteId) return;

    // STEP 1: Block all change events
    swapping.current = true;

    // STEP 2: Save CURRENT sprite's workspace BEFORE clearing
    var saved = save(w);
    if (saved) {
      spriteXml.current[curSprite.current] = saved;
    }

    // STEP 3: Clear workspace (change events blocked by swapping flag)
    try { w.clear(); } catch(_) {}

    // STEP 4: Load NEW sprite's workspace
    var newData = spriteXml.current[props.selectedSpriteId];
    if (newData) {
      load(w, newData);
    }

    // STEP 5: Update current sprite ref
    curSprite.current = props.selectedSpriteId;

    // STEP 6: Unblock change events and notify with a delay
    setTimeout(function() {
      swapping.current = false;
      cbRef.current(getBlocks());
    }, 100);
  ...
  if (st === 'err') {
    return (
      <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',background:'#f0f0f0'}}>
        <div className="window active" style={{width:300}}>
          <div className="title-bar"><div className="title-bar-text">⚠ Error</div></div>
          <div className="window-body has-space" style={{textAlign:'center'}}>
            <p style={{color:'#c0392b',fontWeight:'bold'}}>Blockly failed to load</p>
    ...
    return (
      <div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',background:'#f0f0f0'}}>
        <div style={{textAlign:'center'}}>
          <p style={{fontSize:12,color:'#666',marginBottom:12}}>Loading Blockly...</p>
          <div style={{width:220}}>
